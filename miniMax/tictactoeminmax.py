...
class Minimax:
    ...
    def get_best_move(self, board):
        ...
        for child in board.generate_states(self.player_2):
            eval = self.minimax(child, self.max_depth, False)
            if eval > max_eval:
                max_eval = eval
                best_move = child
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            board = board.make_move(row, col, board.player_1)
            print(board)

            if board.is_win(board.player_1):
                print('Player 1 (x) has won the game!')
                break
            elif board.is_draw():
                print('Game is drawn!')
            ...
            best_move = minimax_algo.get_best_move(board)
            board.position = best_move.position

            print(board)

            if board.is_win(board.player_2):
                print('Player 2 (o) has won the game!')
                break
            elif board.is_draw():
                print('Game is drawn!')
